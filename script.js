...
function updateGame(deltaTime) {
    ...
    if (gameState.cat.some(segment => segment.x === newHead.x && segment.y === newHead.y)) {
        gameOver();
        return;
    }
...
function gameOver(reason) {
    ...
    // 🧹 RESET DELAY GROW (INI PENTING)
    gameState.pendingTailGrow = 0;
    gameState.tailGrowTimer = 0;

    // 🔊 Sound Game Over sesuai sebab
    if (reason === 'water') sounds.splash.play();
    if (reason === 'yarn') sounds.splash.play();
